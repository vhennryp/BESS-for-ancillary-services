def f_BESS_Sv_STK(P_BESS, E_BESS, n_dc, n_ch, BatteryNode, primera_hora, ultima_hora, annual_discount_rate, SoC_i):
    ...
    if E_BESS <= 2 * P_BESS:
        ...
        PS_SoC_max = (E_BESS - P_BESS)/(E_BESS)

    PS_SoC_max = 0.75

    # -------- CONDICIONES INICIALES ------------
    #SoC_i = 0.5                     # SoC_i Inicial
    ...
    for h in range(primera_fila, ultima_fila + 1):
        ...
        # ------------ Switches
        if  P_bid_h >= P_bid_min and SoC >= FR_SoCmin and SoC <= FR_SoCmax:
            SW_FR = 1
        else:
            SW_FR = 0
        ...
        for num_dato in range(primer_min_h, ultimo_min_h + 1):
                    ...
                    if num_dato == 44329:
                        print(f"SoC < FR_SoCmin: {SoC < FR_SoCmin}")
                        print(f"----- num_dato: {num_dato}\t SW_FR_SoCmin:{SW_FR_SoCmin}\t name_policy: {name_policy}")
                    ...
                    # # < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < < <  START ACTIVATION : PEAK-SHAVING
                    # # POLITICA I: BESS Daily Charging

def f_BESS_Sv_FR(P_BESS, E_BESS, n_dc, n_ch, BatteryNode, primera_hora, ultima_hora, annual_discount_rate, SoC_i):
    ...
    t_btw_rf = t_btw_rf                     # Tiempo entre analisis de datos (analisis de datos cada dos segundos)
    P_bid_min = 100                  # kW
    ...
    if BatteryNode.startswith('bus1'):
        CB_Monitored = "Line.CB_101"
        threshold_peak_demand = 170 #15  # kW
    ...
    dss.Command(f"New Load.BESS_PJM_FRM Bus1={BatteryNode} Phases=3 Conn=Wye Model=1 kV=13.8 kW={BatteryPower} kVAR=1 Vmaxpu=1.10 Vminpu=0.90")
    ...
    for h in range(primera_fila, ultima_fila + 1):
        ...
        num_datos = 3600 // t_btw_rf
        primer_min_h = h * num_datos + 1                                         # 1er min. de la h. iterada
        ultimo_min_h = (h + 1) * num_datos
        ...
        for num_dato in range(primer_min_h, ultimo_min_h + 1):
                    ...
                    # #region 
                    # if PS_peak_num_dato > threshold_peak_demand_of and SoC > FR_SoCmin and SoC < FR_SoCmax :  # PS PRIORITY
                    ...
                    # if activate_politica == 0 and h in hrs_day_starts and E_BESS_f < BESS_SoC_max * E_BESS and botom_ch_dayly == 0  and SW_FR_SoCmax == 0:    
                    ...
                    #         E_BESS_f_var = E_BESS_i_var + (f_ch) * (n_ch) * (t_btw_rf / 3600) * (P_BESS) + \
                    #                                       (f_dc) * (( t_btw_rf / 3600) * (- P_BESS) / (n_dc)) - \
                    #                                                   E_BESS_i*b*(t_btw_rf / 3600)
                            
                    #         if E_BESS_f_var > BESS_SoC_max * E_BESS :
                                
                    #             E_BESS_t = BESS_SoC_max * E_BESS - E_BESS_i
                    ...
                    #             dss.Command("Solve")
                    #             dss.Circuit.SetActiveElement("Line.cb_302")     # Activation of Line.cb_302
                    ...
                    if SW_FR == 1 and activate_politica == 0 and SW_PS == 0 and SW_FR_SoCmax == 0 and SW_FR_SoCmin == 0:
                                ...
                                E_BESS_t = (f_ch)*(n_ch)*(t_btw_rf/3600)*(P_bid_h)*( MR_freq.iloc[num_dato- 1, 0]) + \
                                        (f_dc)*((t_btw_rf/3600)*(P_bid_h)*(MR_freq.iloc[num_dato- 1, 0])/(n_dc)) \
                                            - E_BESS_i*b*(t_btw_rf / 3600)
                                ...
                                if SoC < FR_SoCmin and (num_dato) % num_datos == 0:
                                    SW_FR_SoCmin = 1
                                    SW_FR_SoCmax = 0
                                    
                                if SoC > FR_SoCmax and (num_dato) % num_datos == 0:
                                    SW_FR_SoCmax = 1
                                    SW_FR_SoCmin = 0
                ...
                #     if  SW_PS == 0 and activate_politica == 0 and (E_BESS_f == BESS_SoC_max * E_BESS or SW_PS_Rest == 1 or PS_peak_num_dato < threshold_peak_demand_of) :   
                ...
                #         if second_print:
                #             print(f" ******* POLITICA II: h: {h}, num_dato: {num_dato}")
                #             second_print = False
                ...
                #      # > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > FINISH ACTIVATION : PEAK SHAVING
                    ...
                    # if activate_politica == 0 and SW_PS == 1 and PS_peak_num_dato > threshold_peak_demand_of and SW_FR_SoCmax == 0 and SW_FR_SoCmin == 0:
                    ...
                    #         pk_sh_P = PS_peak_num_dato - threshold_peak_demand_of 
                    ...
                    # POLITICA V: BESS Charge SoC Objective
                    if activate_politica == 0 and (SW_FR_SoCmax == 1 or SW_FR_SoCmin == 1): 
                        ...
                        if E_BESS_f + E_BESS_t > SoC_obj * E_BESS and SW_FR_Idle == 0:
                            #print(f"E_BESS_f: {E_BESS_f}, E_BESS_t: {E_BESS_t}, SW_FR_Idle: {SW_FR_Idle}")
                            E_BESS_t = SoC_obj * E_BESS - E_BESS_f
                            P_BESS_t = (f_dc) * (n_dc) * E_BESS_t / (t_btw_rf / 3600) + (1 / n_ch) * (f_ch) * E_BESS_t / (t_btw_rf / 3600)
                            SW_FR_Idle = 1
                        ...
                        if (num_dato) % num_datos == 0:
                            
                            δ_ch_dc_h = (P_BESS)/E_BESS
                            
                            if SW_FR_SoCmin == 1 and SoC == SoC_obj:
                    ...
                    # print(f"h: {h} | num_dato: {num_dato} | ol_SoC: {SoC} | P_BESS_t: {P_BESS_t}")
                    # print(f"df_voltage_droop: \n{df_voltage_droop.head(5)}")
                    # print(f"df_line_currents: \n{df_line_currents.head(5)}")
                    # print(f"df_line_currents_BESS: \n{df_line_currents_BESS.head(5)}")

                        
                    #endregion 
                    # > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > > FINISH ACTIVATION : FREQUENCY REGULATION    
                    
                    # print(f"hr: {h}\t Col_SoC: {SoC}\t P_BESS_t: {P_BESS_t}\t name_policy: {name_policy}\t P_3f_cb_302: {P_3f_cb_302}\t switch_FR: {SW_FR}\t P_bid_h: {P_bid_h}\t switch_PS: {SW_PS}\t PS_peak: {PS_peak_num_dato}\t SW_FR_SoCmin: {SW_FR_SoCmin}\t SW_FR_SoCmax: {SW_FR_SoCmax}\t DSS_Total_Looses_BESS: {DSS_Total_Looses_BESS}\t f4_t: {f4_t} \n")

                    # ------------   STK: Evaluation of Stackable Services
                    
                    # if  SoC > 0.9 or SoC < 0.1:
                    #     print(f"P_bid_h: {P_bid_h}")
                    #     print(f"-------- 0.9 or SoC < 0.1 -------- num_dato: {num_dato}\t SoC: {SoC}\t" )
                    #     print("here")
    ...
    df_E_BESS_t_h = df_info_STK[df_info_STK.index.map(lambda x: (x - 1) % num_datos == 0)][['P_bid_h']]

    # Agregar una nueva columna que almacene el número de índice original
    df_E_BESS_t_h['original_index'] = df_info_STK.index[df_info_STK.index.map(lambda x: (x - 1) % num_datos == 0)]
    ...
    print(f"BEFORE Escalabilidad: annual_revenue: {annual_revenue}")
    ...
    # output_dir = os.path.join(script_dir, 'Results_FR_Test_001')
    ...
    # print("... Executing ... f_f1_payback ... ")
    # Calcular OPEX y CAPEX
    ...
    # Identificar el año de payback fraccionado
    if cumulative_cash_flow < 0:
        f1 = float('inf')  # Si no se alcanza el payback
    else:
        # Sumar los ingresos descontados hasta el año Y-1
        sum_discounted_revenue = sum(
            df_revenues.loc[y - 1, 'Revenue'] / ((1 + annual_discount_rate) ** y) for y in range(1, Y)
        )

        # Ingreso descontado del año Y
        revenue_at_Y_plus_1 = df_revenues.loc[Y, 'Revenue'] / ((1 + annual_discount_rate) ** (Y + 1))
        
        # Aplicar la fórmula corregida de la imagen
        numerator = -Total_Investment + sum_discounted_revenue
        f1 = Y - 1 + abs(numerator / revenue_at_Y_plus_1)

    print(f"annual_revenue: {annual_revenue} f1 (Payback period): {f1:.6f}")
    #endregion
    ...
    return df_info_STK, annual_revenue, f1, f2 ,f3, f4, f5, f6, f_i_1, f_i_2, f_i_3
...
# df_info_STK, annual_revenue, f1, f2 ,f3, f4, f5, f6, f_i_1, f_i_2, f_i_3 = f_BESS_Sv_STK(P_BESS, E_BESS, n_dc, n_ch, BatteryNode, primera_hora, ultima_hora, annual_discount_rate, USD_kW, USD_kWh)

# print(f"df_info_STK: \n{df_info_STK.head()}")
# print(f"annual_revenue: {annual_revenue}")

# print("Valor de f1:", f1)
# print("Tipo de dato de f1:", type(f1))
# # print(f"f1: {f1:.5f}")
# print(f"f1: {f1:.5f}")
# print(f"f2: {f2:.5f}")
# print(f"f3: {f3:.5f}")
